...
module.exports = class Umzug extends _events.EventEmitter {
  ...
  pending() {
    ...
    }).all().then(([all, executed]) => {
      console.log('in pending 180', executed)
  ...
  _run(method, options, rest) {
    ...
    if (options.migrations) {
      console.log('in _run 283')
      return this.execute({
        migrations: options.migrations,
        method: method
      });
    } else {
      console.log('in _run 289', this)
      return rest().bind(this).then(function (migrations) {
        let result = _bluebird.default.resolve().bind(this);
        console.log('in _run 292')

        if (options.to) {
          console.log('in _run 295')
